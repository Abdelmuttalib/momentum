...
export function CreateNewTeamDialog({ companyId }: CreateNewTeamDialogProps) {
...
const UserManagementPage: NextPage<UserManagementPageProps> = ({
  ...
  const { data: companyTeams } = api.team.admin.getAllTeamsByCompanyId.useQuery(
    {
      companyId,
    },
    {
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {companyTeams?.map((team) => (
            <Team key={team.id} {...(team as TTeam)} />
          ))}
        </div>

        <div className="mt-10 lg:mt-20">
          <h4 className="h3 mb-4">Team Info</h4>
          {companyTeams && (
